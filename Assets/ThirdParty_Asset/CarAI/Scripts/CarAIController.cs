...
public class CarAIController : MonoBehaviour
{
    ...
    private void Awake()
    {
        fliplist = GameObject.Find("FlippedList").GetComponent<FlipList>();
      
    }

    private void Start()
    {
        Startcheckpoint = nextCheckpoint.gameObject;
    }
    ...
    IEnumerator CheckForFlippingOver()
    {
        ...
        if (deleteCar)
        {
            ...
            if(deleteCar)
            {
                ...
                fliplist.Addpoint(Startcheckpoint);
                Destroy(gameObject);
            }
        }
    ...
    private void SearchForCheckpoints()
    {
        if (CheckPointSearch && isCarControlledByAI)
        {
            ...
            /*    if(Physics.Raycast(checks[i].position, checks[i].forward, out carHit, maxDistance, seenLayers, QueryTriggerInteraction.Ignore))
                {
                    if (carHit.collider.gameObject != null) UnityEngine.Debug.Log(carHit.collider.gameObject.name); 
                }*/
                #if UNITY_EDITOR
